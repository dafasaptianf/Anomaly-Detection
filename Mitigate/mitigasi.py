...
def monitor_anomalies_and_mitigate(anomalies):
    for anomaly in anomalies:
        ...
        if score < BLOCK_THRESHOLD:
            ...
            # Kurangi repeat limit jika protokol adalah ICMP (lebih agresif)
            if protocol in ["1", "icmp"]:
                repeat_limit = 30  # contoh: lebih cepat blokir ICMP
            else:
                repeat_limit = default_repeat_limit

            # Mitigasi jika jumlah anomali melebihi repeat limit yang ditentukan
            if mac_anomaly_count[src_mac] >= repeat_limit and src_mac not in BLOCKED_MACS:
                ...
                # Reset buffer global (opsional: bisa dibatasi per-mac jika perlu)
                try:
