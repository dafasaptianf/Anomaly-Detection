...
class FlowBytes:
    ...
    def get_min_forward_header_bytes(self) -> int:
        """Calculates the minimum amount of header bytes in the forward direction packets.

        Returns:
            int: The minimum header size in bytes. Returns 0 if no forward packets exist.
        """
        packets = self.feature.packets
        
        # Filter only forward direction packets
        forward_packets = [
            self._header_size(packet) 
            for packet, direction in packets 
            if direction == PacketDirection.FORWARD
        ...
        # Return 0 if no forward packets found, otherwise return min
        return min(forward_packets) if forward_packets else 0
